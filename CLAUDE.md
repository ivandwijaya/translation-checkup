...
**Value-matched pairs & merging:** a value match alone is **not** enough to merge — the two keys must also be **strongly key-similar** (normalized-path similarity ≥ ~0.60). Byte-identical value + weak key similarity (generic short labels like `See More`, `Apply`, `Facilities`, `Enable Now`) is **not** merged; those stay distinct with their own identity. When an Android key and an iOS key share the same (normalized) value, derive workspace/bucket/key from the iOS key's dot-structure — workspace = iOS domain, bucket = workspace-prefixed segment after the domain (e.g. `payment_refund_balance`), key = everything after that segment with element markers kept (`label_about`) — and apply the SAME assignment to both keys with `is merged = TRUE`.
...
**price-alert consolidation:** all price-alert strings (any key containing `pricealert` / `price_alert`, incl. `text_banner_price_alert_*` and `navigation_menu_price_alert`) live in workspace `flight` under a single bucket **`price_alert`** — no feature sub-buckets. Everything else moves into the key: the iOS `revamp` token is dropped; Android feature words (`detail`/`form`/`homepage`/`enable`/`remove`/…) become the key prefix; `text_banner_price_alert_*` keeps a `banner_` key prefix; `navigation_menu_price_alert` → key `navigation_menu_price`.
   - **`experience_text` catch-all:** the `experience` workspace has a single-bucket catch-all `experience_text` for strings that don't belong to a real feature bucket. Two cases fold in here: (a) genuinely isolated strings — any string whose feature bucket would otherwise hold **exactly one row** (no cross-platform partner, no sibling string); and (b) buckets whose name **isn't a meaningful feature** — a negation/filler leading token (e.g. `experience_no`, `experience_non`) or an over-specific one-off phrase (e.g. `experience_product_chain_empty_result`). When flattening a whole bucket in, use the same transform as any bucket-flatten: prepend the bucket's suffix (everything after `experience_`) to the existing key (`experience_no` key `internet_title` → `experience_text` key `no_internet_title`); merged pairs stay identical because both sides share bucket+key. For a lone isolated string the key is just its full leaf after `experience_`, template symbols stripped (`iospoe.experience_and_%@` → key `and`). Cross-platform pairs (same key on both platforms) and any real feature shared by ≥2 strings keep their normal feature bucket — they are **not** moved here.
